...
function Slider() {
    ...
    useEffect(() => {
        ...
        const items = dataSlider.map((_, index) => ({
            position: index,
            el: carouselElement.querySelector(
    ...
    return (
        <div
            id="default-carousel"
            className="relative w-full pt-[32px]"
            data-carousel="slide"
            ref={carouselRef}
        >
            <div className="relative h-56 overflow-hidden md:h-[50rem]">
                {dataSlider.map((item, index) => (
                    ...
                    >
                        ...
                        <div className="absolute font-bold opacity-[1.1] text-white md:text-[1rem] text-[0.5rem] px-4 leading-relaxed m-0 -translate-x-1/2 left-1/2 md:bottom-[10%] bottom-[15%] md:w-[60%] w-[85%]">
                            <p className="text-center md:text-[1.2rem] text-[0.7rem]">
                                {item.title}
                            </p>
                            <p className="text-center md:text-[1.2rem] text-[0.7rem]">
                                {item.subTitle}
