import { addCartItem, updateCartQuantity } from '@/redux/cartSlice';
...
export default function ProductDetail() {
    ...
    if (
        !userData ||
        !userData?.imageDetail ||
        userData?.imageDetail?.length === 0
    ) {
        return null;
    }

    const getSizeOpacity = (size) => {
        const selectedSizeObj = userData.sizes.find(
            (item) => item.size === size,
        );
                ...
                <div className="xl:w-1/3 w-full xl:ps-5 mx-2">
                    <div>
                        <div className="mb-4 border-b border-gray-300 pb-4">
                            <h3 className="mb-4 name-product text-3xl">
                                {userData?.name}
                            </h3>
                            <p className="text-[1rem] text-[#5b5b5b]">
                                Mã hàng : {userData?.series}
                            </p>
                            <p className="text-black font-bold text-[1.1rem]">
                                {formatCurrency(userData?.price)}
                            </p>
                        </div>
                        <div className="">
                            ...
                            <div className="flex items-center mb-5 pb-2">
                                <label className="mr-4 text-[1rem]">
                                    Số lượng
                                </label>
